/* CrowRules YouTube Live bridge — Tacoma Nights single-stream + matching chat v4 */
(function(){
  ...
  const target=()=>document.getElementById('youtube-live-player');
  ...
  function render(data){
    ...
    if(!data?.live||!data.videoId){
      if(state){state.textContent='OFFLINE';state.className='status'}
    ...
    if(state){state.textContent='LIVE NOW';state.className='status live'}
    if(lastVideoId===data.videoId&&root.querySelector('iframe'))return;
  ...
  async function check(){
    try{const r=await fetch(ENDPOINT+'?site='+encodeURIComponent(SITE),{cache:'no-store'});const data=await r.json();render(data);return data}
    catch(e){const state=document.getElementById('liveState');if(state){state.textContent='LIVE CHECK UNAVAILABLE';state.className='status error'}return null}
  }
