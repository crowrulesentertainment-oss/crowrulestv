...
async function failoverState(id){const r=await sb.from("spectrum_broadcast_failover_state").select("*").eq("ceremony_id",id).maybeSingle();const e=document.getElementById("failoverStatus");if(e)e.textContent=r.data?("PROGRAM "+(r.data.program_source_name||"None")+" · "+(r.data.program_status||"unknown")+" · BACKUP "+(r.data.backup_source_name||"None")+" · "+(r.data.backup_status||"unknown")+" · "+r.data.failover_state):"No broadcast sources configured.";return r.data;}
async function doFailover(kind){if(!current)return;const st=await failoverState(current.id);if(!st)return;const u=(await sb.auth.getUser()).data.user;const source=kind==="failover"?st.backup_source_id:st.program_source_id;if(!source)return;const program=kind==="failover"?st.backup_source_id:st.program_source_id;const preview=kind==="failover"?st.program_source_id:st.backup_source_id;await sb.from("spectrum_broadcast_switches").insert({ceremony_id:current.id,program_source_id:program,preview_source_id:preview,switched_by:u.id,note:kind==="failover"?"Automatic/manual backup failover":"Primary restoration"});await sb.from("spectrum_broadcast_failover_events").insert({ceremony_id:current.id,failed_source_id:kind==="failover"?st.program_source_id:st.backup_source_id,backup_source_id:kind==="failover"?st.backup_source_id:st.program_source_id,event_type:kind,reason:kind==="failover"?"Program source unavailable":"Primary restored",operator_id:u.id});await failoverState(current.id);await loadBroadcast(current.id);}
async function loadBroadcast(id){const r=await sb.from("spectrum_broadcast_sources").select("*").eq("ceremony_id",id).eq("is_active",true).order("sort_order");const ps=document.getElementById("programSource"),vs=document.getElementById("previewSource");if(!ps||!vs)return;ps.innerHTML=(r.data||[]).map(x=>'<option value="'+x.id+'">'+esc(x.name)+" · "+esc(x.source_type)+'</option>').join("");vs.innerHTML=ps.innerHTML;const st=await sb.from("spectrum_broadcast_state").select("*").eq("ceremony_id",id).maybeSingle();if(st.data){ps.value=st.data.program_source_id||"";vs.value=st.data.preview_source_id||"";}const el=document.getElementById("broadcastStatus");if(el)el.textContent=st.data?("PROGRAM: "+(st.data.program_source_name||"None")+" · PREVIEW: "+(st.data.preview_source_name||"None")):"No switch recorded.";}
async function takeProgram(){if(!current)return;const p=document.getElementById("programSource")?.value,v=document.getElementById("previewSource")?.value;if(!p)return;const u=(await sb.auth.getUser()).data.user;await sb.from("spectrum_broadcast_switches").insert({ceremony_id:current.id,program_source_id:p,preview_source_id:v||null,switched_by:u.id});await loadBroadcast(current.id);}
